...
limiter = Limiter(
    get_remote_address,
    app=app,
    default_limits=["15000 per day", "5000 per hour", "3000 per minute"],
    storage_uri="memory://",
)
...
def task(task_name):
    ...
    instructions = {
        ...
        'cct_cold': {
            ...
            выбор предусмотрен штраф. Награда за карту - это сумма, которую вы получите за каждую перевернутую «хорошую» карту, если среди перевернутых карт не окажется ни одной "плохой".
            Размер проигрыша - это штраф, который вы получите, если перевернёте хотя бы одну "плохую" карту, независимо от того, какие другие карты Вы перевернули. Сверху у вас есть информация по текущему раскладу: размер выигрыша за каждую перевернутую
